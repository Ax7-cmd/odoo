...
QUnit.test("simple title", async (assert) => {
    assert.expect(1);
    env.services.title.setParts({ zopenerp: "Scolabs" });
    assert.strictEqual(env.services.title.current, "Scolabs");
});

QUnit.test("add title part", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Scolabs", chat: null });
    assert.strictEqual(env.services.title.current, "Scolabs");
    env.services.title.setParts({ action: "Import" });
    assert.strictEqual(env.services.title.current, "Scolabs - Import");
});
...
QUnit.test("all at once", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Scolabs", action: "Import" });
    assert.strictEqual(env.services.title.current, "Scolabs - Import");
...
QUnit.test("get title parts", async (assert) => {
    ...
    assert.deepEqual(parts, { zopenerp: "Scolabs", action: "Import" });
    parts.action = "Export";
    assert.strictEqual(env.services.title.current, "Scolabs - Import"); // parts is a copy!
});
